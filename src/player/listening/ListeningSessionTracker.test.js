...
test('a successful hard replacement keeps frozen A evidence despite B media timeupdates', () => {
  const context = fixture()
  context.tracker.actualPlay(trackA, { positionMs: 0, durationMs: 100000 })
  context.advance(30000)
  const replacement = context.tracker.prepareTransition({ type: 'next', reason: 'user_next' })
  context.tracker.freezeForReplacement(replacement, { positionMs: 30000, durationMs: 100000 })
  context.tracker.position(500)
  context.tracker.position(1000)
  context.tracker.seek(1000, 2000, 240000)
  context.tracker.commitTransition(replacement, trackB, { positionMs: 2000, durationMs: 240000 })
  const aSkip = context.events.find((event) => event.type === 'skip' && event.trackId === 'netease-1')
  const bPlay = context.events.find((event) => event.type === 'play' && event.trackId === 'netease-2')
  assert.equal(aSkip.positionMs, 30000)
  assert.equal(aSkip.durationMs, 100000)
  assert.equal(aSkip.metadata.listenDurationMs, 30000)
  assert.equal(aSkip.metadata.completionRatio, 0.3)
  assert.equal(bPlay.positionMs, 2000)
  assert.equal(bPlay.durationMs, 240000)
})

test('a second replacement cannot overwrite the first frozen A evidence', () => {
  const context = fixture()
  context.tracker.actualPlay(trackA, { positionMs: 0, durationMs: 100000 })
  context.advance(30000)
  const first = context.tracker.prepareTransition({ reason: 'track_replaced' })
  context.tracker.freezeForReplacement(first, { positionMs: 30000, durationMs: 100000 })
  const second = context.tracker.prepareTransition({ reason: 'track_replaced' })
  context.tracker.freezeForReplacement(second, { positionMs: 2000, durationMs: 240000 })
  context.tracker.commitTransition(second, trackB, { positionMs: 0, durationMs: 240000 })
  const aSkip = context.events.find((event) => event.type === 'skip' && event.trackId === 'netease-1')
  assert.equal(aSkip.positionMs, 30000)
  assert.equal(aSkip.durationMs, 100000)
  assert.equal(aSkip.metadata.completionRatio, 0.3)
...
test('a stale hard replacement cannot fail or clear the newer exact transition', () => {
  const context = fixture()
  context.tracker.actualPlay(trackA, { positionMs: 10000, durationMs: 100000 })
  const first = context.tracker.prepareTransition({ type: 'previous', reason: 'user_previous' })
  context.tracker.freezeForReplacement(first, { positionMs: 10000 })
  const second = context.tracker.prepareTransition({ type: 'previous', reason: 'user_previous' })
  assert.equal(context.tracker.failReplacement(first), false)
  assert.equal(context.tracker.commitTransition(first, trackB), null)
  assert.ok(context.tracker.commitTransition(second, trackB, { positionMs: 0, durationMs: 100000 }))
  assert.deepEqual(context.events.map((event) => event.type), ['play', 'previous', 'skip', 'play'])
  assert.equal(context.events.at(-1).trackId, 'netease-2')
})
...
test('rapid hard requests commit only the newest exact transition and never play the stale target', () => {
  ...
  assert.equal(context.tracker.commitTransition(firstTransition, trackB), null)
  assert.equal(context.tracker.failReplacement(firstTransition), false)
  assert.equal(requests.isCurrent(secondRequest, trackC), true)
  assert.ok(context.tracker.commitTransition(secondTransition, trackC, { positionMs: 0, durationMs: 100000 }))

  assert.deepEqual(context.events.map((event) => event.type), ['play', 'previous', 'skip', 'play'])
  assert.equal(context.events.some((event) => event.trackId === 'netease-2' && event.type === 'play'), false)
  assert.equal(context.events.at(-1).trackId, 'netease-3')
})
