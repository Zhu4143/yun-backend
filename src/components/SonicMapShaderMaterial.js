...
export const MapShaderMaterial = shaderMaterial(
  ...
  `
    ...
    void main() {
      ...
      vCoreMask = 1.0 - smoothstep(uCoreRadius, uCoreRadius + 2.2, centerDist);
      ...
      idleElevation *= mix(1.0, 0.04, vCoreMask);
      ...
      audioElevation *= mix(1.0, uCoreResponse, vCoreMask);
      ...
      for(int i = 0; i < 10; i++) {
        if(uRipples[i].isActive > 0.0) {
           ...
           rippleElevation += rPulse * elevationScale * mix(1.0, 0.03, vCoreMask);
           if (uRipples[i].rippleType > 0.5) {
  ...
  `
    ...
    void main() {
      ...
      if (isTop) {
         ...
         finalColor = mix(finalColor, cBase1 * 1.35 + vec3(0.0, 0.025, 0.04), vCoreMask * 0.78);
         ...
         finalColor = mix(finalColor, cBase1 * 1.2, vCoreMask * 0.72);
      ...
      finalColor += vec3(0.22, 0.85, 1.0) * vCoreMask * step(0.985, random(vInstancePos * 3.7 + uTime * 0.03)) * 0.5;
