  ...
  try {
    ...
    return sendJson(res, 200, {
      ...
      lyrics: {
        ...
        lines: parseSyncedLyrics(lyrics.raw),
      },
    });
  } catch (error) {
...
async function handleNeteaseAudio(req, res) {
  try {
    ...
    if (!playableUrl) {
      return sendJson(res, 404, { ok: false, error: "No playable url for this song" });
    }
...
- reply 不要显示 JSON，不要固定模板，不要总说“我陪你听”，不要总问“是不是想起谁了”。`;
    ...
    const rawFinalReply = String(decision.reply || "嗯，我在。你慢慢说。").trim();
